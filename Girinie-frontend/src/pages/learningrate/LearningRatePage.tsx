...
export default function LearningRatePage() {
  ...
  const weekNumber = Math.floor((selectedWeekStart.getTime() - firstMonday.getTime()) / (7 * 24 * 60 * 60 * 1000)) + 1;
  ...
  return (
    <div className="min-h-screen bg-[#FFF6BF]">
      ...
      <div className="ml-64 space-y-10 p-8">
        <h1 className="mb-4 text-2xl font-bold">학습률 상세 보기</h1>
            ...
            <div className="rounded-xl border bg-white shadow-sm">
              ...
              <div className="px-4 py-4">
                <div className="mb-3 flex items-center justify-between text-sm text-gray-700">
                  ...
                  <span>{displayYear % 100}년 {displayMonth}월 {weekNumber}주차</span>
                  <button 
                    className="rounded px-2 py-1 hover:bg-gray-100 cursor-pointer"
          ...
          <div className="space-y-4">
            {child &&
              Object.entries(CATEGORY_LABELS).map(([key, label]) => {
                const level = (child as any)[key] as number;
                const percent = Math.min(100, Math.max(0, (level / 4) * 100));
                return (
