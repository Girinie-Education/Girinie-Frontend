...
const SettingPage: React.FC = () => {
  ...
  if (!children.length)
    return <div className="p-10 text-gray-600">등록된 자녀가 없습니다.</div>;
        ...
        <div className="relative w-full max-w-5xl mx-auto bg-yellow-100 rounded-lg shadow-md p-8 overflow-hidden">
          ...
          <div className="flex gap-6 justify-center items-center">
            {visibleItems.map((item) =>
              item === "ADD" ? (
                <button
                  key="add-card"
                  className="bg-white hover:bg-gray-100 cursor-pointer transition rounded-lg shadow-md p-6 w-full max-w-[300px]
                             sm:max-w-[340px] md:max-w-[360px] h-[350px] flex flex-col items-center justify-center flex-shrink-0"
                  onClick={() => {
                    /* TODO: 자녀 추가 모달로 이동 처리 */
                  }}
                >
          ...
          {page > 0 && (
            <button
              ...
              className="absolute top-1/2 left-2 -translate-y-1/2 w-10 h-10 bg-white rounded-full shadow-md
          ...
          {page < totalPages - 1 && (
            <button
              onClick={() => setPage((p) => p + 1)}
              className="absolute top-1/2 right-2 -translate-y-1/2 w-10 h-10 bg-white rounded-full shadow-md
                         flex items-center justify-center"
            >
              <ChevronRight />
            </button>
